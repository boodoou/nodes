...
> Maven是由java开发的，即基于Java，所以Maven工具必须依赖于JDK
...
执行 mvn clean命令时运行前两个生命周期阶段，而执行mvn post-clean命令时运行所有三个生命周期阶段
...
当通过命令调用某一个阶段时，会执行该阶段（包含）之前的所有阶段；如：

```
mvn package
```

此命令会执行从validate到package的所有阶段。
...
pom.xml中通常可以通过profile标签定义（指定）多个配置文件，如：
...
也可以通过Maven设置文件（setting.xml）激活的文件：
...
<dependencies>
    ...
    <dependency>
        ...
        <systemPath>${basedir}\src\lib\ldapjdk.jar</systemPath> <!--项目根目录下的lib文件夹下，此处引入的是本地依赖-->
    </dependency> 
</dependencies>
```
